...
main(){
  ...
  output ""
  echo -en "* Nitro Domain (nitro.local) : "
  read -r DOMAIN_INPUT

  [ -z "$DOMAIN_INPUT" ] && DOMAIN="nitro.local" || DOMAIN=$DOMAIN_INPUT

  output ""
  echo -en "* CMS Domain (cosmic.local) : "
  read -r CMS_DOMAIN_INPUT

  [ -z "$CMS_DOMAIN_INPUT" ] || CMS_DOMAIN=$CMS_DOMAIN_INPUT
...
update_config(){
  if [[ "$answer" =~ [Yy] ]]; then
    ...
    # asset.url for http
    sed -i -e "s|ASSET_URL|http://${DOMAIN}|g" dist/renderer-config.json
  fi
  ...
  # Externaltexts.json
  sed -i -e 's|${asset.url}/bundled/ExternalTexts.json|${gamedata.url}/json/ExternalTexts.json|g' dist/renderer-config.json
  ...
  # furnidata.url
  sed -i -e 's|${gamedata.url}/json/FurnitureData.json|${asset.url}/bundled/furniture/json/FurnitureData.json|g' dist/renderer-config.json
  ...
  sed -i -e 's|${gamedata.url}/json/FigureData.json|${asset.url}/bundled/clothes/json/FigureData.json|g' dist/renderer-config.json

  # avatar.figuremap.url
  sed -i -e 's|${gamedata.url}/json/FigureMap.json|${asset.url}/bundled/clothes/json/FigureMap.json|g' dist/renderer-config.json
  ...
  # avatar.asset.url
  sed -i -e 's|${asset.url}/bundled/figure/%libname%.nitro|${asset.url}/bundled/clothes/nitro/%libname%.nitro|g' dist/renderer-config.json
...
summary(){
  ...
  output "********************************************************"
  output ""
  output "You Nitro folder : /var/www/$DOMAIN"
  output ""
